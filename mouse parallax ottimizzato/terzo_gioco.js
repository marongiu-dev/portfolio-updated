...
this.aggiungiBottone_animation(copiaArray,arrayTrueoFalse,array_of_ids_for_button_hiding,i);
...
//Animation on top screen

aggiungiBottone_animation:function(nuova_parola,arrayTrueoFalse,id_of_edge,i)
{
  console.log('funzionaaaaaaaaaa') 
 //create a new div
 ...
 divs.setAttribute("class","div_animation");
 ...
 bottoni.setAttribute("class","anim_pushable");
...
/*  //set span id & class
 ...
 var grid1 = document.getElementById('second_box');
 grid1.appendChild(divs);

 //add the new button element to html
 var tuttiBottoni = Array.from(document.getElementsByClassName('div_animation'));
 
 tuttiBottoni.forEach(bottone =>{
     
   bottone.classList.add('button_clone_pos');
   //bottone.style.visibility='hidden';
 bottone.appendChild(bottoni);

 //add the 1st new spans in the button element
 var tuttispan = Array.from(document.getElementsByClassName('anim_pushable'));
...
buttonClone:function(){
  ...
  clones.classList.add(`split_animation_topscreen_${i}`);
  clones.style.visibility='visible';

},

click_button_clone_light: function(){
  let clones = Array.from(document.getElementsByClassName('div_animation')[i]);
